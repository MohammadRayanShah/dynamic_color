...
export class ColorGridSelectComponent
  ...
  public grid = computed((): string[][] => {
    ...
    console.log('this.validatedRGBs', this.validatedRGBs);

    const items = [...this.items, ...this.validatedRGBs];

    return chunk(items, this._itemsPerRow);
  });

  ngOnChanges(changes: SimpleChanges) {
    console.log('validatedRGBs changes', changes['validatedRGBs'].currentValue);
    this.validatedRGBs = changes['validatedRGBs'].currentValue;
  ...
  ngOnInit() {
    // Set ARIA roles on initialization
  ...
  private _onKeydown(event: KeyboardEvent) {
    switch (event.keyCode) {
      case UP_ARROW: {
        this._moveFocus(-this._itemsPerRow);
        break;
      }
      case DOWN_ARROW: {
        this._moveFocus(this._itemsPerRow);
